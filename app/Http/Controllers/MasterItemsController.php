...
class MasterItemsController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $data_search = $data_search->select('kode', 'nama', 'jenis', 'harga_beli', 'laba', 'supplier', 'photo')->orderBy('id')->get();
    ...
    public function formView($method, $id = 0)
    {
        if ($method == 'new') {
            ...
            $selected_kategori = $item->kategoriItems->pluck('id')->toArray();
        }
        $kategori_list = KategoriItem::all();
        $data['item'] = $item;
        $data['method'] = $method;
    ...
    public function formSubmit(Request $request, $method, $id = 0)
    {
        ...
        if ($request->hasFile("photo")) {
            if ($method == 'edit' && $data_item->photo) {
                Storage::delete('public/' . $data_item->photo);
            }

            $file = $request->file("photo");
            $fileName = time() . "." . $file->getClientOriginalExtension();
            $file->storeAs('public/product', $fileName);
            $data_item->photo = 'product/' . $fileName;
        }
